...
function renderStores(storeList) {
  ...
  storeList.forEach(store => {
    ...
    card.innerHTML = `
      ...
      <div class="tags">
        ${store.tags.map(tag => `<span class="tag blue">${tag}</span>`).join("")}
      </div>
      ...
      <a href="store-detail.html?id=${store.id}">
...
/* document.querySelectorAll(".tab").forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    if (text === "All") currentStatus = "all";
    if (text === "Operating") currentStatus = "operating";
    if (text === "Restocking") currentStatus = "restocking";
    if (text === "Maintenance") currentStatus = "maintenance";
